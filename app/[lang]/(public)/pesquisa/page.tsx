      ...
      <section className="bg-white border-b border-slate-100 py-6 px-4 md:px-6 shadow-sm">
        <div className="max-w-[1100px] mx-auto">
          ...
          <form method="GET" action={`/${lang}/pesquisa`} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 items-center">
            ...
            <select name="pais" value={paisUI} onChange={(e) => setPaisUI(e.target.value)} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-700 outline-none focus:border-emerald-500">
            ...
            <select name="categoria" defaultValue={catParam} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-700 outline-none focus:border-emerald-500">
            ...
            {mostrarDistritos && (
              <select name="distrito" defaultValue={distParam} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-700 outline-none focus:border-emerald-500">
                <option value="">{isEn ? 'All Districts' : 'Todos os Distritos'}</option>
                {distritosPT.map(d => <option key={d} value={d}>{d}</option>)}
              </select>
            )}

            <select name="idade" defaultValue={idadeParam} className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-700 outline-none focus:border-emerald-500">
            ...
            <button type="submit" className="w-full py-2.5 bg-slate-900 text-white font-bold text-sm rounded-lg hover:bg-slate-800 transition-colors">
              {isEn ? 'Update' : 'Atualizar'}
            </button>
            
            {filtrosAtivos && (
              <Link href={`/${lang}/pesquisa`} className="text-sm font-bold text-red-600 hover:text-red-700 text-center sm:text-left">
              ...
              return (
                <div key={campo.id} className="flex flex-col bg-white rounded-2xl overflow-hidden border border-slate-200 relative shadow-sm hover:shadow-md transition-shadow">
                  ...
                  <div className="relative h-48 w-full overflow-hidden">
                    <img src={campo.imagem} alt={nomeCampo} className="w-full h-full object-cover" />
                  ...
                  <div className="flex flex-col p-5 flex-1 pointer-events-none">
                    <span className="text-xs font-bold text-emerald-700 uppercase mb-1">📍 {localCampo}</span>
                    <h3 className="text-lg font-black text-slate-900 leading-tight mb-2">{nomeCampo}</h3>
                    <p className="text-sm text-slate-500 font-medium mb-4">{isEn ? 'Age' : 'Idade'}: <span className="text-slate-900">{idadeCampo}</span></p>

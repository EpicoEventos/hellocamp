    ...
    <main className="min-h-screen bg-slate-200 py-12 px-4 font-sans text-black selection:bg-yellow-200">
      <div className="max-w-[900px] mx-auto">
        ...
        <div className="mb-10 text-center">
          <span className="bg-slate-900 text-white px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest shadow-sm">
          ...
          <p className="text-slate-600 font-medium mt-3 max-w-2xl mx-auto">
            {isEn 
        ...
        <form onSubmit={handleSubmeterTudo} className="bg-white shadow-2xl p-8 md:p-16 text-black leading-relaxed rounded-sm font-serif">
          ...
          <div className="space-y-4 font-serif text-[15px]">
            <h2 className="text-xl font-bold text-center uppercase tracking-widest mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8 mt-8 bg-gray-50 p-8 border border-gray-200 rounded-lg">
              <div className="flex flex-col md:col-span-2"><label className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-1">{isEn ? 'Company Name (Billing) *' : 'Nome da Empresa (Faturação) *'}</label><input required type="text" className={inputClass} value={form.nomeEmpresa} onChange={e => setForm({...form, nomeEmpresa: e.target.value})} placeholder={isEn ? "Legal designation" : "Designação legal"} /></div>
              <div className="flex flex-col"><label className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-1">{isEn ? 'VAT Number *' : 'NIF *'}</label><input required type="text" className={inputClass} value={form.nif} onChange={e => setForm({...form, nif: e.target.value})} placeholder={isEn ? "Tax Identification Number" : "Número de Identificação Fiscal"} /></div>
              <div className="flex flex-col"><label className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-1">{isEn ? 'Legal Form *' : 'Forma Jurídica *'}</label><input required type="text" className={inputClass} value={form.formaJuridica} onChange={e => setForm({...form, formaJuridica: e.target.value})} placeholder={isEn ? "Ex: LLC, Association..." : "Ex: Lda, Associação..."} /></div>
              <div className="flex flex-col"><label className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-1">{isEn ? 'Contact Person *' : 'Pessoa de Contacto *'}</label><input required type="text" className={inputClass} value={form.pessoaContacto} onChange={e => setForm({...form, pessoaContacto: e.target.value})} placeholder={isEn ? "Full name" : "Nome completo"} /></div>
              <div className="flex flex-col"><label className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-1">{isEn ? 'Phone Number *' : 'Número de Telefone *'}</label><input required type="text" className={inputClass} value={form.telefone} onChange={e => setForm({...form, telefone: e.target.value})} /></div>
              <div className="flex flex-col md:col-span-2"><label className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-1">{isEn ? 'Billing Address *' : 'Morada Fiscal *'}</label><input required type="text" className={inputClass} value={form.morada} onChange={e => setForm({...form, morada: e.target.value})} placeholder={isEn ? "Street, number, floor" : "Rua, número, andar"} /></div>
              <div className="flex flex-col"><label className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-1">{isEn ? 'Postal Code and City *' : 'Código Postal e Cidade *'}</label><input required type="text" className={inputClass} value={form.codigoPostal} onChange={e => setForm({...form, codigoPostal: e.target.value})} placeholder={isEn ? "0000-000 City" : "0000-000 Localidade"} /></div>
              <div className="flex flex-col"><label className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-1">{isEn ? 'Commercial E-mail *' : 'E-mail Comercial *'}</label><input required type="email" className={inputClass} value={form.emailContacto} onChange={e => setForm({...form, emailContacto: e.target.value})} /></div>
              <div className="flex flex-col"><label className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-1">{isEn ? 'Booking E-mail *' : 'E-mail para Reservas *'}</label><input required type="email" className={inputClass} value={form.emailReservas} onChange={e => setForm({...form, emailReservas: e.target.value})} /></div>
              <div className="flex flex-col"><label className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-1">{isEn ? 'Website (Optional)' : 'Website (Opcional)'}</label><input type="text" className={inputClass} value={form.website} onChange={e => setForm({...form, website: e.target.value})} /></div>
              <div className="flex flex-col"><label className="text-xs font-bold uppercase tracking-wider text-gray-500 mb-1">{isEn ? 'Data Protection Officer (GDPR) *' : 'Responsável de Dados (RGPD) *'}</label><input required type="text" className={inputClass} value={form.responsavelRGPD} onChange={e => setForm({...form, responsavelRGPD: e.target.value})} placeholder={isEn ? "Officer's name" : "Nome do responsável"} /></div>
            </div>
            ...
            <div className="bg-emerald-50 border border-emerald-200 p-8 my-8 font-sans rounded-xl shadow-inner">
              <label className="text-sm font-bold text-emerald-900 block mb-3 uppercase tracking-widest">
              ...
              <p className="text-sm font-medium text-emerald-700 mt-4">
          ...
          <div className="space-y-6 text-[15px] text-justify pt-12">
            <h3 className="font-bold text-xl uppercase tracking-widest border-b border-black pb-2 mb-8">
          ...
          <div className="space-y-12 font-sans text-sm text-slate-800">
            ...
            <div className="bg-gray-50 p-6 rounded-xl border border-gray-200">
              <h3 className="font-black text-lg mb-4 text-black uppercase border-l-4 border-[#EBA914] pl-3">
            ...
            <div className="bg-blue-50 p-6 rounded-xl border border-blue-100">
              <h3 className="font-black text-lg mb-4 text-blue-950 uppercase border-l-4 border-blue-600 pl-3">
            ...
            <div className="bg-amber-50 p-6 rounded-xl border border-amber-100">
              <h3 className="font-black text-lg mb-4 text-amber-950 uppercase border-l-4 border-amber-500 pl-3">
            ...
            <div className="bg-white p-6 rounded-xl border border-gray-200">
              <h3 className="font-black text-lg mb-4 text-black uppercase border-l-4 border-gray-400 pl-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16 mt-16 pt-12 border-t-2 border-black font-sans">
            <div>
              <h4 className="font-black mb-6 uppercase tracking-wider text-black text-sm">{isEn ? 'For HelloCamp' : 'Pela HelloCamp'}</h4>
              <p className="font-serif text-xl italic text-gray-500 mb-2">{isEn ? 'HelloCamp Administration' : 'Administração HelloCamp'}</p>
              <p className="text-sm font-medium border-t border-gray-200 mt-6 pt-4">{isEn ? 'Date:' : 'Data:'} {dataAtual}</p>
            </div>

            <div>
              <h4 className="font-black mb-6 uppercase tracking-wider text-black text-sm">{isEn ? 'For the Partner' : 'Pelo Parceiro'}</h4>
              <div className="space-y-4">
                <div className="flex flex-col">
                  <label className="text-xs font-bold uppercase mb-1 text-gray-500">{isEn ? 'Digital Signature Name *' : 'Nome da Assinatura Digital *'}</label>
                  <input required type="text" className="border-b-2 border-black bg-transparent outline-none py-2 text-xl font-serif italic focus:border-[#EBA914] transition-colors" value={form.assinaturaNome} onChange={e => setForm({...form, assinaturaNome: e.target.value})} placeholder={isEn ? "Signatory's name" : "Nome de quem assina"} />
                </div>
                <div className="flex flex-col">
                  <label className="text-xs font-bold uppercase mb-1 text-gray-500">{isEn ? 'Role / Position *' : 'Cargo *'}</label>
                  <input required type="text" className="border-b border-gray-300 bg-transparent outline-none py-2 focus:border-black transition-colors" value={form.assinaturaCargo} onChange={e => setForm({...form, assinaturaCargo: e.target.value})} placeholder={isEn ? "Ex: Managing Partner" : "Ex: Sócio-Gerente"} />
                </div>
                <p className="text-sm font-medium border-t border-gray-200 mt-6 pt-4">{isEn ? 'Date:' : 'Data:'} {dataAtual}</p>
              </div>
          ...
          <div className="bg-slate-900 p-8 rounded-2xl text-white font-sans mt-16 shadow-2xl relative overflow-hidden">
            <h3 className="text-2xl font-black mb-2 relative z-10">{isEn ? 'Final Step: Access Account' : 'Último Passo: Conta de Acesso'}</h3>
            <p className="text-sm text-slate-400 mb-8 relative z-10">{isEn ? 'Define your credentials to access your Organizer Dashboard.' : 'Defina as credenciais para aceder ao seu Dashboard de Organizador e gerir este campo.'}</p>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">{isEn ? 'Login Email (Access) *' : 'Email de Login (Acesso) *'}</label>
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">{isEn ? 'Define a Password *' : 'Defina uma Password *'}</label>
                <input type="password" required minLength={6} value={auth.passwordAcesso} onChange={e => setAuth({...auth, passwordAcesso: e.target.value})} className="w-full p-4 bg-slate-800 border border-slate-700 rounded-xl text-white outline-none focus:border-[#EBA914] transition-colors" placeholder={isEn ? "Minimum 6 characters" : "Mínimo 6 caracteres"} />
              </div>
            </div>
